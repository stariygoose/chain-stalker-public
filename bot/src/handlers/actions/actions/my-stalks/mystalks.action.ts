...
export class MyStalksAction extends Action {
    ...
    this.bot.action(MyStalksAction.handler, async (ctx) => {
      try {
        ...
        await ctx.reply(message, {
          ...
          reply_markup: {
            inline_keyboard: [
              [
                {
                  text: Buttons.menuBtn.text,
                  callback_data: Buttons.menuBtn.callback_data,
                },
              ],
            ],
          },
        ...
        if (error instanceof ApiError) {
          await this._menuCommand.showMenu(ctx, error.botMessage);
          return;
        }

        this._logger.error(
          `Unexpected error when user ${ctx.from?.id} was trying to see them stalks: ${(error as Error).message}`,
        );
  ...
  private formatSubscriptionMessage(ctx: MyContext, sub: Subscription): string {
    ...
    switch (target.type) {
      ...
      default:
        this._logger.warn(
          `Unknown subscription target type: ${target["type"]}`,
        );
